...
CREATE TABLE IF NOT EXISTS `User` (
  ...
  `firstseen_date` DATE NULL,
  `lastseen_date` DATE NULL,
...
CREATE TABLE IF NOT EXISTS `RegisteredUsers` (
  ...
  `User_tracking_id` INT NOT NULL,
  `role_id` INT NULL,
  PRIMARY KEY (`tracking_id`),
  UNIQUE INDEX `email_UNIQUE` (`email` ASC) VISIBLE,
  INDEX `fk_RegisteredUsers_User_idx` (`User_tracking_id` ASC) VISIBLE,
  INDEX `role_id_idx` (`role_id` ASC) VISIBLE,
  CONSTRAINT `fk_RegisteredUsers_User`
    FOREIGN KEY (`User_tracking_id`)
    REFERENCES `User` (`tracking_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
